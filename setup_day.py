...
next_day = max([int(d.name.split('-')[1]) for d in base_dir.joinpath(str(default_year)).iterdir()]) + 1

parser = argparse.ArgumentParser(description='Setup a new day for Advent of Code')
parser.add_argument('--year', type=int, help='The year to setup for', default=default_year)
parser.add_argument('--day', type=int, help='The day to setup', default=next_day)
